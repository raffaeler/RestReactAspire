...
public class PatientServiceEndpointTests : IClassFixture<TestWebApplicationFactory<RestReactAspire.PatientService.PatientServiceMarker>>
{
    ...
    {
        var response = await _client.GetAsync("/api/patients", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();

        var list = await response.Content.ReadFromJsonAsync<PatientListResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(list);
    ...
    public async Task CreatePatient_ReturnsCreated_WithHateoasLinks()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/patients", request, TestContext.Current.CancellationToken);

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);

        var patient = await response.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        var response = await _client.GetAsync($"/api/patients/{Guid.NewGuid()}", TestContext.Current.CancellationToken);
    ...
    public async Task CreateAndGetPatient_RoundTrips()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/patients", request, TestContext.Current.CancellationToken);
        createResponse.EnsureSuccessStatusCode();

        var created = await createResponse.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var getResponse = await _client.GetAsync($"/api/patients/{created.Id}", TestContext.Current.CancellationToken);
        getResponse.EnsureSuccessStatusCode();

        var retrieved = await getResponse.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdatePatient_ReturnsOk_WithUpdatedData()
    {
        ...
        var createResp = await _client.PostAsJsonAsync("/api/patients", createReq, TestContext.Current.CancellationToken);
        var created = await createResp.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var updateReq = new UpdatePatientRequest("Updated", "Name", new DateOnly(1995, 3, 15), "updated@example.com", "555-2222");
        var updateResp = await _client.PutAsJsonAsync($"/api/patients/{created.Id}", updateReq, TestContext.Current.CancellationToken);
        updateResp.EnsureSuccessStatusCode();

        var updated = await updateResp.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeletePatient_ReturnsNoContent()
    {
        ...
        var createResp = await _client.PostAsJsonAsync("/api/patients", request, TestContext.Current.CancellationToken);
        var created = await createResp.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var deleteResp = await _client.DeleteAsync($"/api/patients/{created.Id}", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NoContent, deleteResp.StatusCode);

        var getResp = await _client.GetAsync($"/api/patients/{created.Id}", TestContext.Current.CancellationToken);
    ...
    public async Task UpdatePatient_ReturnsNotFound_WhenMissing()
    {
        ...
        var response = await _client.PutAsJsonAsync($"/api/patients/{Guid.NewGuid()}", request, TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response = await _client.DeleteAsync($"/api/patients/{Guid.NewGuid()}", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        await _client.PostAsJsonAsync("/api/patients", new CreatePatientRequest("SearchAlpha", "One", new DateOnly(1990, 1, 1), "alpha@example.com", "555-0001"), TestContext.Current.CancellationToken);
        await _client.PostAsJsonAsync("/api/patients", new CreatePatientRequest("SearchBeta", "Two", new DateOnly(1991, 2, 2), "beta@example.com", "555-0002"), TestContext.Current.CancellationToken);

        var response = await _client.GetAsync("/api/patients?search=SearchAlpha", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await _client.GetAsync("/api/patients?search=ZZZNonExistent", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    public async Task GetPatients_WithSortParams_ReturnsSortedResults()
    {
        ...
        var create1 = await _client.PostAsJsonAsync("/api/patients", req1, TestContext.Current.CancellationToken);
        var p1 = await create1.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
        var create2 = await _client.PostAsJsonAsync("/api/patients", req2, TestContext.Current.CancellationToken);
        var p2 = await create2.Content.ReadFromJsonAsync<PatientResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(p1);
        Assert.NotNull(p2);

        var response = await _client.GetAsync("/api/patients?sortBy=lastName&sortDirection=asc&search=Sort", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    public async Task GetPatients_WithSortDesc_ReturnsSortedDescending()
    {
        ...
        var response = await _client.GetAsync("/api/patients?sortBy=lastName&sortDirection=desc&search=Desc", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    {
        for (int i = 0; i < 15; i++)
            await _client.PostAsJsonAsync("/api/patients", new CreatePatientRequest("SortPag", $"Last{i}", new DateOnly(1990, 1, 1), $"sortpag{i}@example.com", $"555-{i:D4}"), TestContext.Current.CancellationToken);

        var response = await _client.GetAsync("/api/patients?search=SortPag&pageSize=10&sortBy=email&sortDirection=desc", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    {
        for (int i = 0; i < 15; i++)
            await _client.PostAsJsonAsync("/api/patients", new CreatePatientRequest("PagSearch", $"Last{i}", new DateOnly(1990, 1, 1), $"pag{i}@example.com", $"555-{i:D4}"), TestContext.Current.CancellationToken);

        var response = await _client.GetAsync("/api/patients?search=PagSearch&pageSize=10", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
