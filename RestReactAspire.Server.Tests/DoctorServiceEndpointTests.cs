...
public class DoctorServiceEndpointTests : IClassFixture<TestWebApplicationFactory<RestReactAspire.DoctorService.DoctorServiceMarker>>
{
    ...
    {
        var response = await _client.GetAsync("/api/doctors", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();

        var list = await response.Content.ReadFromJsonAsync<DoctorListResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(list);
    ...
    public async Task CreateDoctor_ReturnsCreated_WithHateoasLinks()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/doctors", request, TestContext.Current.CancellationToken);

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);

        var doctor = await response.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        var response = await _client.GetAsync($"/api/doctors/{Guid.NewGuid()}", TestContext.Current.CancellationToken);
    ...
    public async Task CreateAndGetDoctor_RoundTrips()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/doctors", request, TestContext.Current.CancellationToken);
        createResponse.EnsureSuccessStatusCode();

        var created = await createResponse.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var getResponse = await _client.GetAsync($"/api/doctors/{created.Id}", TestContext.Current.CancellationToken);
        getResponse.EnsureSuccessStatusCode();

        var retrieved = await getResponse.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateDoctor_ReturnsOk_WithUpdatedData()
    {
        ...
        var createResp = await _client.PostAsJsonAsync("/api/doctors", createReq, TestContext.Current.CancellationToken);
        var created = await createResp.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var updateReq = new UpdateDoctorRequest("Updated", "Name", "Surgery", "updated@hospital.com", "555-4444");
        var updateResp = await _client.PutAsJsonAsync($"/api/doctors/{created.Id}", updateReq, TestContext.Current.CancellationToken);
        updateResp.EnsureSuccessStatusCode();

        var updated = await updateResp.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeleteDoctor_ReturnsNoContent()
    {
        ...
        var createResp = await _client.PostAsJsonAsync("/api/doctors", request, TestContext.Current.CancellationToken);
        var created = await createResp.Content.ReadFromJsonAsync<DoctorResponse>(cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(created);

        var deleteResp = await _client.DeleteAsync($"/api/doctors/{created.Id}", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NoContent, deleteResp.StatusCode);

        var getResp = await _client.GetAsync($"/api/doctors/{created.Id}", TestContext.Current.CancellationToken);
    ...
    public async Task UpdateDoctor_ReturnsNotFound_WhenMissing()
    {
        ...
        var response = await _client.PutAsJsonAsync($"/api/doctors/{Guid.NewGuid()}", request, TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response = await _client.DeleteAsync($"/api/doctors/{Guid.NewGuid()}", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        await _client.PostAsJsonAsync("/api/doctors", new CreateDoctorRequest("SearchDoc", "Alpha", "Dermatology", "searchdoc@hospital.com", "555-9001"), TestContext.Current.CancellationToken);
        await _client.PostAsJsonAsync("/api/doctors", new CreateDoctorRequest("OtherDoc", "Beta", "Surgery", "otherdoc@hospital.com", "555-9002"), TestContext.Current.CancellationToken);

        var response = await _client.GetAsync("/api/doctors?search=SearchDoc", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    {
        await _client.PostAsJsonAsync("/api/doctors", new CreateDoctorRequest("Doc", "One", "UniqueSpecialty", "doc1@hospital.com", "555-9003"), TestContext.Current.CancellationToken);
        await _client.PostAsJsonAsync("/api/doctors", new CreateDoctorRequest("Doc", "Two", "OtherSpecialty", "doc2@hospital.com", "555-9004"), TestContext.Current.CancellationToken);

        var response = await _client.GetAsync("/api/doctors?search=UniqueSpecialty", TestContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
