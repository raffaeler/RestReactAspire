...
globs:
  - "RestReactAspire.Server/Endpoints/StatisticsEndpoints.cs"
  - "RestReactAspire.Server/Models/StatisticsDto.cs"
  - "RestReactAspire.Server/Telemetry/StatisticsTelemetry.cs"
...
Located in `RestReactAspire.Server/Endpoints/StatisticsEndpoints.cs`, registered under `/api/statistics`.
...
```
- Each response includes HATEOAS links to all statistics endpoints plus main resource lists.
- DTOs in `RestReactAspire.Server/Models/StatisticsDto.cs`.

### Adding a New Statistic
1. Add DTO records to `StatisticsDto.cs`.
2. Add endpoint method in `StatisticsEndpoints.cs`.
3. Add telemetry counter in `StatisticsTelemetry.cs`.
4. Register the link in `RootEndpoints.cs` and in `GetStatisticsLinks()`.
