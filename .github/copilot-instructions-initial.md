...
Step 11. Modify the seeding code to generate more data, to have 100 patients, 30 doctors and 200 exams. Then create a new page on the frontend to display some statistics about the data. There must be at least 3 different charts: a pie chart showing the distribution of patients by age group, a bar chart showing the number of exams per doctor, a line chart showing the number of exams over time and a line chart showing the average duration of each type of exam over time. Use recharts library (https://recharts.org/en-US/) to implement the charts on the frontend, and ensure that the API provides the necessary endpoints to retrieve the data for the charts.
Step 12. Create the required Copilot skills under the folder `.github/skills`. Each skill must be named `SKILL.md` under a folder named `.github/skills/{name-of-the-skill}`. Skill subdirectory names should be lowercase and use hyphens for spaces. You create as many skills are required to cover all the maintenance and the evolution of this project over time. You use this instructions and the steps already implemented as a starting point for the skills. Future requests to AI will have to use the skills. Once you have done that, you create a new copilot-instructions-proposal.md file with the new instructions that will be shorter because of the presence of the skills. Ensure each skill has the appropriate front-matter used by the Copilot.
